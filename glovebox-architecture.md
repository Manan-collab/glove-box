...
**Foundations + Slices 1–5 (Auth, Cars, Expenses, Analytics, Friends) are all built.** Image/receipt upload and vehicle-data-API enrichment are the only deliberately deferred pieces — see below. **An ad hoc sixth feature, Excel import/export (`src/modules/data-io/`), was added after Slice 5** on top of that — not part of the original slice plan, see the module notes below and its [Section 22 checklist](#module-data-importexport). **A seventh ad hoc addition, Car Notes + a car-detail-page redesign, followed after that** (migration `20260824091029_add_car_health_and_notes`: `usageTag`/`insuranceExpiryDate` on `Car`, an `ExpenseCategory.PURCHASE` value, a new append-only `CarNote` model/`src/modules/notes/` module, and a client-only, never-fabricated car-health/acquisition-date layer) — see the module notes below and its [Section 22 checklist](#module-notes). This section is the source of truth for "what's real right now"; treat every other section as the target design, not the current state.
...
- `src/modules/data-io/` — **built**, ad hoc Excel import/export (not part of the original slice plan — see [Section 22](#module-data-importexport)). `DataIoModule` imports `CarsModule`/`ExpensesModule` and every create/update goes through `CarsService`/`ExpensesService` directly, the same ownership-checked methods manual entry uses, never raw Prisma. **Quick fix worth noting**: `ExpensesModule` never had `exports: [ExpensesService]` — a pre-existing gap invisible until something outside `ExpensesModule` itself needed to inject the service; adding the one-line `exports` array was all it took, not a debugging saga on the scale of the numbered Gotchas above. **One sheet per car** (revised mid-design from an original "Cars"+"Expenses" two-sheet pair, based on user feedback) — each sheet has a car-details header block (key/value rows 1–13), a blank separator, then an expenses table (row 15+, `workbook/workbook-columns.ts`). Putting every expense physically on its own car's sheet removes any need for a cross-sheet linking key, **sidestepping gotcha #1 (no `$transaction`, ever) by design rather than working around it**. Sheet name is `"<Model> <Year>"` truncated to Excel's 31-char limit and de-duplicated with `(2)`/`(3)`… suffixes — a human-navigation label only; the "Car ID" header cell alone decides create-vs-update. Import (`importWorkbook`) is sequential and best-effort, consistent with gotcha #1: a bad sheet or expense row becomes a `{sheet, row?, message}` error (`RowErrorDto`) and is skipped, never aborting the rest of the import; a car-level validation failure skips that sheet's expenses too (nowhere valid to attach). An ownership mismatch on an existing Car ID/Expense ID (someone else's, or nonexistent) is converted to a row/sheet error rather than left to throw — e2e-tested with a cross-user "attack" workbook (user B importing a sheet referencing user A's real Car ID) that errors cleanly and leaves user A's data untouched. Validation is deliberately **stricter than manual entry**: enums (Fuel Type, Transmission, Body Type, Category) are matched case-insensitively against the same lists the UI dropdowns use and rejected with the valid-options list otherwise, even though the DTOs accept any non-empty string for those fields — Excel is a much higher-error-rate input surface than a dropdown. Numeric ranges mirror the DTOs' `class-validator` rules (Year 1900–current+1, Power ≥1, Odometer ≥0, Amount/Litres/FuelPricePerLitre >0) even though import bypasses the global `ValidationPipe` entirely (straight through the service layer) — added deliberately once it was noticed this was initially missing, since without it import could create data the manual form would reject. Dates (`workbook/workbook-validators.ts`) accept a real Excel date cell, an ISO/common string, or a raw numeric Excel serial (a frequent copy-paste artifact) — all three normalize identically, via the standard Dec-30-1899 epoch (the industry-standard "off by one day pre-March-1900" tradeoff from Excel's own leap-year bug, not a bug here). Soft rule: a FUEL row with only one of `litres`/`fuelPricePerLitre` set is rejected as ambiguous rather than silently accepted. No new transactional/race-handling pattern was needed beyond the ownership-mismatch-to-row-error conversion above — unlike Friends' P2002-retry pattern, Car/Expense have no unique-constrained fields today. Template download ships a "Read Me" sheet (plain-text instructions, first tab, skipped on import case-insensitively) plus an "Example Car" sheet with realistic placeholder data and real Excel dropdown validation on the enum columns. `exceljs` was added (over the lighter `xlsx`/SheetJS) specifically for its native dropdown/data-validation write support; `@types/multer` was added too — first file-upload code in the app, so no multer types existed yet even though `@nestjs/platform-express` already bundles multer itself. Routes: `GET /data-io/export`, `GET /data-io/template`, `POST /data-io/import` (5MB limit via `FileInterceptor`), all behind `JwtAuthGuard`.
- `src/modules/notes/` — **built**, ad hoc addition alongside the car-detail redesign (see the intro to this section and [Section 22](#module-notes)). `NotesModule` imports `CarsModule`, giving `NotesService` the exact same ownership pattern `ExpensesService` uses: `create`/`findAllForCar` call `carsService.findOneForUser` first (nested-route ownership check), `remove` checks ownership directly via `findFirst({ where: { id, car: { userId } } })` (the flat-route pattern). `CarNote` (`id`, `carId`, `body`, `createdAt`, `@@map("car_notes")`) is deliberately append-only — no update endpoint, just add/delete. No pagination (expected low volume). Routes: `POST /cars/:carId/notes`, `GET /cars/:carId/notes`, `DELETE /notes/:id`, all behind `JwtAuthGuard`.
- Tests: `auth.service.spec.ts` + `cars.service.spec.ts` + `expenses.service.spec.ts` + `analytics.service.spec.ts` + `friends.service.spec.ts` + `users.service.spec.ts` + `data-io.service.spec.ts` + `notes.service.spec.ts` (unit, **65** total) and one e2e spec per module including `data-io.e2e-spec.ts` and `notes.e2e-spec.ts` (real app + real DB, **64** total across 8 suites). Every e2e suite proves cross-user isolation over real HTTP calls (the architecture doc's stated top testing priority, Section 16). The Analytics suite specifically hand-computes a fixture (two FUEL expenses + one SERVICE expense with known odometer readings) and asserts the API's `totalSpend`/`trackedKm`/`costPerKm`/`monthlySpend` match the manual calculation exactly — satisfying that module's stricter "Done when" bar. The Friends suite is the longest (19 tests): full request → duplicate-conflict → reverse-request-conflict → reject → re-request → accept → view-garage → unfriend → re-verify-404 cycle across three users. The Data Import/Export suite adds 13 unit + 7 e2e tests, including the cross-user attack-workbook case above and a re-import-the-same-export-file idempotency check (0 created, 1 updated on the second pass). The Notes suite (5 unit + 6 e2e) proves the same two-user ownership isolation on all three routes; `cars.e2e-spec.ts` also gained one round-trip test confirming `usageTag`/`insuranceExpiryDate` survive a create→read cycle. `test/jest-e2e.json` sets `maxWorkers: 1` (parallel workers caused Neon connection contention) and `testTimeout: 15000` (raised from Jest's 5000ms default — some Friends-flow tests make 4+ sequential real network round-trips, which routinely exceeded the default under normal Neon latency; this was a genuine latency issue, confirmed by rerunning green with the longer timeout, not a logic bug).
...
- `app/dashboard/page.tsx` — page-head has a time-of-day greeting + "Your Garage" + an honest car-count subtitle (still no fabricated spend/health stats here — that's what the dedicated Analytics page is for now). Now also has an "Import / Export" button next to "+ Add Car", opening `DataIoDialog`.
- `features/data-io/data-io-dialog.tsx`, `lib/data-io-api.ts`, `hooks/use-data-io.ts` — **built**, ad hoc Excel import/export (see [Section 0](#0-current-implementation-status) and [Section 22](#module-data-importexport)). The dialog offers three actions — export, download template, import from a chosen `.xlsx` file — and renders the `ImportResultDto` result as a MUI `Alert` (created/updated counts, plus a scrollable list of any per-row/per-sheet errors). A successful import invalidates both the `['cars']` and `['expenses']` TanStack Query keys, so the Garage page updates with no manual refresh. This is **the first file-upload and first file-download code in the entire app**, which is why `lib/api-client.ts`'s private `request()` was split: `fetchWithRefresh()` (the fetch + silent-401-refresh-and-retry logic) and `throwApiError()` were pulled out as shared exports, since the multipart upload (`FormData`, deliberately no explicit `Content-Type` so the browser sets the correct boundary itself) and the Blob download (`URL.createObjectURL` + a synthetic `<a download>` click) can't go through `request()` — it hardcodes JSON headers and JSON-parses the body — but still need the same resilience to an access token expiring mid-action as every other call in the app. Verified end-to-end with a real authenticated Playwright session: template download with real Excel dropdown validation on the enum columns, importing a new car + 2 expenses showed "1 car created, 2 expenses created, 1 issue" with a genuine per-row error for an intentionally-bad Fuel Type value, the Garage page updated without a manual refresh, and re-importing the exact same exported file showed "0 created, 1 updated" — proving idempotency.
- `app/dashboard/cars/[id]/page.tsx` — **fully rewritten** into three tabs (MUI `Tabs`/`Tab`: Overview / Expenses / History), replacing the earlier single flat page (header still matches `.cd-header`: gradient photo, badges for fuel/body/power; same Edit/Delete as before). Header title is now `{make} {model}` (year moved into the subtitle), plus badges that only render when real data backs them: a `usageTag` chip, an owned/added pill (`getOwnedLabel`), and a health badge (`getHealthScore`) — and a new "Add Note" button next to "+ Expense". **Overview tab**: the existing `useCarAnalytics` stat row (Total spent / Cost per km / Tracked, unchanged), a Recent Expenses panel and a Quick Facts panel side by side, and a Spending Breakdown panel via the shared `CategoryBreakdownBars` (see the Analytics bullet above). **Expenses tab**: the expense list plus a client-side category filter dropdown and expenses grouped under month headers, plus Import/Export buttons opening the existing whole-garage `DataIoDialog` — **deliberate simplification**: this imports/exports the whole garage, not just this car (true per-car export would need backend changes), flagged rather than silently scoped down. **History tab**: a client-side merge of expenses + notes sorted by date descending, no new backend endpoint.
- `lib/car-facts.ts` — pure, client-side-only derivation functions computed from already-fetched Car/Expense data, no new backend endpoints for any of it: `getAcquisition(car, expenses)` (resolves "Bought" plus a price from the earliest `PURCHASE` expense if one exists, else falls back to "Added" from `car.createdAt`), `getOwnedLabel(acquisition)`, `getLastServiceKmAgo(car, expenses)` (km since the most recent `SERVICE` expense with a recorded odometer reading), `getInsuranceDaysLeft(car)`, `getHealthScore(car, expenses)`. The health score is two independent 0–100 components (service recency against a fixed 10,000 km assumption, insurance-expiry banding) averaged over whichever components have real data — **if neither has data it returns `null` and no badge renders at all; deliberately never fabricated**, the same principle Analytics already applies to cost-per-km/tracked-km (Section 0's Analytics bullet). Scoped to the car detail page only — the Garage dashboard's earlier "Garage Health" list stays removed and does not come back via this.
- `lib/notes-api.ts` + `hooks/use-notes.ts` (`useCarNotes`/`useCreateNote`/`useDeleteNote`) — same shape as `expenses-api.ts`/`use-expenses.ts`. `features/cars/add-note-dialog.tsx` is a minimal single-textarea dialog, no edit (matches `CarNote`'s append-only backend design).
- `app/dashboard/analytics/page.tsx` — garage-wide stat row, a `@mui/x-charts` `BarChart` for monthly spend (themed automatically off the CSS-variable palette, no manual color mapping needed beyond referencing `var(--mui-palette-primary-main)`), a category breakdown (plain `sx`-styled bars, not a chart library — matches `.spend-bar-row` closely enough that a chart would be overkill), and a car-comparison table. The stat row and category breakdown are built from shared components in `features/analytics/analytics-ui.tsx` (`StatCard`, `Panel`, and `CategoryBreakdownBars` — the last one extracted from what used to be inline JSX here, now reused by the car detail page's Overview tab's "Spending Breakdown" panel too). Verified against the exact same hand-computed fixture as the backend e2e test — the UI numbers match the manual calculation, not just "the API returned something."
...
- `features/expenses/` — `expense-categories.ts` (now 9 categories with icons — `PURCHASE` (🔑, "Purchase") added later alongside the backend enum change — matching the design's `.cat-grid` exactly), `expense-form-schema.ts` (Zod, same coercion pattern as the car form, also updated for `PURCHASE`), `expense-form-dialog.tsx` (category grid + `useWatch` — not `watch`, which React Compiler can't safely memoize — to conditionally render category-specific fields: litres/price-per-litre/station for fuel, workshop/work-performed for service, workshop/what-broke for repair, brand/size for tyres), `expense-row.tsx` (matches `.expense-row`; **fixed a real bug**: `{expense.odometerKm && (...)}` rendered a literal `0` instead of hiding the row for a legitimate `odometerKm: 0` reading — e.g. a `PURCHASE` expense logged at the moment of buying, at 0 km — the same class of falsy-but-valid-value bug this doc already tracks elsewhere; changed to `!= null`).
...
12. **Excel import/export** — ✅ done. An ad hoc feature added after Slice 5, not part of the original slice plan above — a one-sheet-per-car workbook design (revised once from an original two-sheet "Cars"+"Expenses" design based on user feedback) that sidesteps the no-`$transaction` constraint (gotcha #1) by construction. See the [Data Import/Export checklist](#module-data-importexport) and [Section 0](#0-current-implementation-status).
13. **Car Notes + car-detail-page redesign** — ✅ done. Another ad hoc addition, after Excel import/export — a `usageTag`/`insuranceExpiryDate` on `Car`, an `ExpenseCategory.PURCHASE` value, append-only per-car notes (`src/modules/notes/`), and a client-side-only, never-fabricated car-health/acquisition-date layer (`lib/car-facts.ts`) feeding a redesigned three-tab (Overview/Expenses/History) car detail page. See the [Notes checklist](#module-notes) and [Section 0](#0-current-implementation-status).
...
- [x] `usageTag`/`insuranceExpiryDate` optional fields + a `CarNote` relation, added later via the `20260824091029_add_car_health_and_notes` migration — see the [Notes checklist](#module-notes)
...
- [x] Car detail page — rewritten into three tabs (Overview/Expenses/History): analytics stat row, Recent Expenses + Quick Facts panels, Spending Breakdown via the shared `CategoryBreakdownBars`, a category-filtered expense list grouped by month, a notes+expenses History merge, edit + delete-with-confirmation retained
- [x] `lib/car-facts.ts` health-score/acquisition-date derivation, entirely client-side and `null` when no real data backs it (never fabricated) — surfaced as an optional usage-tag chip / owned-or-added pill / health badge in the header
...
**Backend**
- [x] Expense migration + `ExpenseCategory` enum (9 categories: `BATTERY` was already one more than this doc's original 7, and `PURCHASE` was added later still — see [Section 0](#0-current-implementation-status) — to record what a car cost to buy)
...
- [x] Expense history list (on the car detail page, matching `.expense-row`; fixed a bug where an `odometerKm: 0` reading rendered a literal `0` instead of hiding the row — see [Section 0](#0-current-implementation-status))
...
*Ad hoc feature added after Slice 5 — not part of the original vertical-slice plan (see [Section 0](#0-current-implementation-status) and [Section 21](#21-step-by-step-development-roadmap) item 12). Went through a plan-mode design revision: the original design was a two-sheet "Cars"+"Expenses" workbook; the final design is one sheet per car, chosen specifically to avoid needing a cross-sheet linking key.*

**Backend**
- [x] `DataIoModule` (imports `CarsModule`/`ExpensesModule`; exposed a pre-existing gap — `ExpensesModule` wasn't exporting `ExpensesService` — fixed with a one-line `exports` addition, see [Section 0](#0-current-implementation-status))
- [x] Workbook writer (`workbook-writer.ts`): one sheet per car (`"<Model> <Year>"`, truncated/de-duplicated), car-details header block + expenses table, real Excel dropdown validation on enum columns
- [x] Workbook reader/validators (`workbook-reader.ts`, `workbook-validators.ts`): stricter-than-manual-entry enum + numeric-range validation, three-format date coercion (real date cell, string, or numeric serial), a soft `litres`/`fuelPricePerLitre` both-or-neither rule
- [x] Import service (`upsertCar`/`upsertExpense` via the real `CarsService`/`ExpensesService`, never raw Prisma) — sequential, best-effort, one bad sheet/row recorded as a `RowErrorDto` and skipped rather than aborting the request
- [x] Ownership-mismatch handling: an existing Car ID/Expense ID that isn't the importing user's becomes a row/sheet error, not a thrown exception
- [x] Template endpoint with a "Read Me" sheet (skipped on import, case-insensitively) + an "Example Car" sheet
- [x] Routes: `GET /data-io/export`, `GET /data-io/template`, `POST /data-io/import` (5MB limit via `FileInterceptor`), all behind `JwtAuthGuard`
- [x] Tests: 13 unit (`data-io.service.spec.ts`) + 7 e2e (`data-io.e2e-spec.ts`), including a cross-user "attack" workbook (User B referencing User A's real Car ID) that errors cleanly and leaves User A's data untouched, and a re-import-same-file idempotency check

**Frontend**
- [x] `DataIoDialog` (`features/data-io/data-io-dialog.tsx`) — export / download-template / import-from-file, result shown as a MUI `Alert` with per-row/per-sheet error detail
- [x] `lib/data-io-api.ts` / `hooks/use-data-io.ts` — typed API functions + TanStack Query hooks, import invalidates `['cars']` and `['expenses']`
- [x] "Import / Export" button wired into `app/dashboard/page.tsx` next to "+ Add Car"
- [x] `lib/api-client.ts` refactor: `fetchWithRefresh`/`throwApiError` extracted as shared exports so the app's first file-upload and first file-download code still gets the silent-401-refresh-and-retry resilience `request()` provides everywhere else

**Integration**
- [x] Connected — verified via a real authenticated Playwright session: template download with working Excel dropdowns, importing a new car + 2 expenses with one intentionally-bad Fuel Type value showed "1 car created, 2 expenses created, 1 issue" with the real per-row error, and the Garage page updated with no manual refresh
- [x] Idempotency verified: re-importing the exact same exported file showed "0 created, 1 updated"
- [x] Cross-user isolation verified over real HTTP in `data-io.e2e-spec.ts` — an attack workbook referencing another user's Car ID errors instead of mutating their data

**Done when**: ✅ met — a user can export their garage to Excel, re-import it idempotently, bulk-create/update cars and expenses from a spreadsheet with per-row error reporting instead of an all-or-nothing failure, and cannot touch another user's data by referencing their IDs in an uploaded workbook.

### MODULE: NOTES
*Ad hoc addition alongside the car-detail-page redesign (see [Section 0](#0-current-implementation-status)) — small, append-only per-car notes, shipped in the same migration (`20260824091029_add_car_health_and_notes`) as the `usageTag`/`insuranceExpiryDate` Car fields and the `ExpenseCategory.PURCHASE` value documented under the [Garage/Cars](#module-garage--cars) and [Expenses](#module-expenses) checklists above.*

**Backend**
- [x] `CarNote` model (`id`, `carId`, `body`, `createdAt`, `@@map("car_notes")`), append-only — no update endpoint, just add/delete
- [x] `NotesModule`/`NotesController`/`NotesService` (imports `CarsModule`), `CreateNoteDto`
- [x] Ownership: `create`/`findAllForCar` go through `carsService.findOneForUser` first (nested-route pattern); `remove` checks via `findFirst({ where: { id, car: { userId } } })` — the same split Expenses uses
- [x] Routes: `POST/GET /cars/:carId/notes`, `DELETE /notes/:id`, all behind `JwtAuthGuard`; no pagination (expected low volume)
- [x] Tests: `notes.service.spec.ts` (5 unit) + `notes.e2e-spec.ts` (6 e2e, ownership isolation between two users)

**Frontend**
- [x] `lib/notes-api.ts` + `hooks/use-notes.ts` (`useCarNotes`/`useCreateNote`/`useDeleteNote`), same shape as the Expenses equivalents
- [x] `features/cars/add-note-dialog.tsx` — minimal single-textarea dialog, no edit (matches the append-only backend)
- [x] Surfaced on the car detail page's new History tab (merged with expenses, sorted by date descending) — see the [Garage/Cars checklist](#module-garage--cars) above for the rest of that redesign

**Integration**
- [x] Cross-user isolation verified over real HTTP in `notes.e2e-spec.ts`

**Done when**: ✅ met — notes can be added, listed, and deleted per car, and are correctly scoped to the owning user's cars only.
